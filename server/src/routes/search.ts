...
import { searchMemories, conversationalSearch } from '../services/search-pipeline.js';
import type { SearchResult, SearchResponse, SearchSuggestion, Memory, Tag, FamilyMemberRef } from '@family-memories/shared';

const router = Router();

// AI-powered hybrid search (vector + keyword + rerank)
router.post('/', async (req, res) => {
  try {
    ...
    // If AI pipeline returned no results, fall back to SQLite LIKE search
    if (response.results.length === 0) {
      const fallback = sqliteFallbackSearch(query, filters, resultLimit);
    ...
    logger.error({ err }, 'AI search failed, falling back to SQLite');
    try {
      const { query, filters, limit: maxResults } = req.body;
      const resultLimit = Math.min(50, Math.max(1, maxResults ?? 20));
      const fallback = sqliteFallbackSearch(query, filters, resultLimit);
      ...
      logger.error({ err: fallbackErr }, 'Fallback search also failed');
      res.status(500).json({ code: 'SEARCH_ERROR', message: 'Search failed' });
    }
  }
});

// Conversational search with RAG-generated answer
router.post('/conversational', async (req, res) => {
  try {
    ...
    const resultLimit = Math.min(10, Math.max(1, maxResults ?? 5));

    const result = await conversationalSearch(query, resultLimit);
    ...
    res.status(500).json({ code: 'SEARCH_ERROR', message: 'Conversational search failed' });
  }
});

// Autocomplete suggestions (unchanged — fast SQLite LIKE queries)
...
function sqliteFallbackSearch(
  query: string,
  filters: { types?: string[]; person_ids?: string[]; tags?: string[]; date_range?: { from?: string; to?: string } } | undefined,
  resultLimit: number,
): SearchResponse {
  const db = getDb();
