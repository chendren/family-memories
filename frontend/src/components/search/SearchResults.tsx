    ...
    <MemoryGrid className={className}>
      {results.map((result) => (
        <div key={result.memory.id} className="relative">
          ...
          />
          <div className="absolute top-2 right-2 px-2 py-0.5 rounded-full text-[10px] font-medium bg-terracotta-50 text-terracotta-600 border border-terracotta-200 font-body">
            {Math.round(result.score * 100)}%
          </div>
          {result.snippet && (
            <div className="mt-1 px-4 pb-2">
              <p className="text-xs text-walnut-500 line-clamp-2 font-body">{result.snippet}</p>
            </div>
          )}
        </div>
      ))}
